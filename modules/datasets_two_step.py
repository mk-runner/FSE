...
class IuxrayFinetuneDataset(FinetuneBaseDataset):  # finetune and inference phase
    def __getitem__(self, idx):
        example = self.examples[idx]
        image_id = example['id']
        image_path = example['image_path']
...
class MimiccxrFinetuneDataset(FinetuneBaseDataset):  # finetune and inference phase
    def __getitem__(self, idx):
        example = self.examples[idx]
        image_id = example['id']
        image_path = example['image_path']
        ...
        # report
        report = example['report']
        # if self.split in ['train', 'vad']:
        #     report_ids = self.tokenizer.encode('[BOS] ' + report + " [EOS]").ids[: self.max_seq_length]
        # else:  # val or test
        #     report_ids = self.tokenizer.encode(report).ids
        report_ids = self.tokenizer.encode('[BOS] ' + report + " [EOS]").ids[: self.max_seq_length]
        report_len = len(report_ids)
        report_masks = [1] * report_len
