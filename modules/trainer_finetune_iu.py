...
class BaseTrainer(object):
    ...
    def _load_checkpoint(self, load_path):
        # 'visual_global.head.0.weight'

        load_path = str(load_path)
        self.logger.info("Loading checkpoint: {} ...".format(load_path))
        checkpoint = torch.load(load_path)
        # if self.args['task'] == 'finetune' and self.args['data_name'] == 'iu_xray':
        #     global_weight = checkpoint['state_dict']['visual_global.head.0.weight']
        #     checkpoint['state_dict']['visual_global.head.0.weight'] = torch.cat([global_weight, global_weight], dim=1)
        # self.model.load_state_dict(checkpoint['state_dict'], strict=True)
        # path = '/home/miao/data/download/R2GEN/model_iu_xray.pth'
...
class PTrainer(BaseTrainer):
    def __init__(self, model, metric_ftns, optimizer, args, lr_scheduler, train_dataloader, val_dataloader,
                 test_dataloader, logger, task, runner, is_save_checkpoint, **kwargs):
...
class PretrainTester(object):
    def __init__(self, model, train_dataloader, val_dataloader, test_dataloader, logger, args, mimic_train_loader, **kwargs):
        self.train_dataloader = train_dataloader
        self.val_dataloader = val_dataloader
        self.test_dataloader = test_dataloader
    ...
    def get_specific_knowledge_mimic_cxr(self, id2image, save_file_name):
        # get id2report dict
        id2report = {}
...
class FTrainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, (image_ids, images, report_ids, report_masks, sk_ids, sk_masks) in enumerate(
            ...
            self.optimizer.zero_grad()
            ret = self.model(images, report_ids, report_masks, sk_ids, sk_masks, mode='train')
            ret['all_loss'].backward()
            train_loss += ret['all_loss'].cpu().detach().item()
            torch.nn.utils.clip_grad_value_(self.model.parameters(), 0.1)
            self.optimizer.step()
            if batch_idx % 2000 == 0 or batch_idx + 1 == len(self.train_dataloader):
                print(f"Epoch {epoch}, step {batch_idx}/{len(self.train_dataloader)}, "
                      f"all_loss: {ret['all_loss'].cpu().detach().item():0.4}, "
                      f"lr: {self.optimizer.param_groups[0]['lr']}")
                self.logger.info(f"Epoch {epoch}, step {batch_idx}/{len(self.train_dataloader)}, "
            ...
            del ret, images, report_ids, report_masks
            torch.cuda.empty_cache()
        log = {'train_loss': train_loss / len(self.train_dataloader), 'epoch': epoch}
