...
from .datasets_two_step import IuxrayFinetuneDataset, MimiccxrFinetuneDataset
...
class FinetuneLoader(DataLoader):
    def __init__(self, args, tokenizer, split, shuffle, drop_last=False):
        ...
        if args['data_name'] == 'iu_xray':
            self.dataset = IuxrayFinetuneDataset(args, tokenizer, self.split, transform=self.transform)
        elif args['data_name'] == 'mimic_cxr':
            self.dataset = MimiccxrFinetuneDataset(args, tokenizer, self.split, transform=self.transform)
        else:
            raise ValueError
